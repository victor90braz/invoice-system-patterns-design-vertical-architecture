...
class TestCancelledState(TestCase):
    ...
    def test_approve_invoice_from_cancelled(self):
        invoice = InvoiceFactory.create(invoice_number="021", total_value=100.0, state=self.cancelled_state)
    ...
    def test_cancel_invoice_from_cancelled(self):
        invoice = InvoiceFactory.create(invoice_number="022", total_value=0.0, state=self.cancelled_state)
        cancelled = CancelledState()
    ...
    def test_pay_invoice_from_cancelled(self):
        invoice = InvoiceFactory.create(invoice_number="023", total_value=0.0, state=self.cancelled_state)
        cancelled = CancelledState()
    ...
    def test_validate_transition_from_cancelled(self):
        invoice = InvoiceFactory.create(invoice_number="024", total_value=0.0, state=self.cancelled_state)
        cancelled = CancelledState()
